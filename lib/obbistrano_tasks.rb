    ...
    task :install, :roles =>[:host] do 
      ...
      begin
        with_user("root", "#{root_pass}") do 
          run "rm -f /etc/nginx/sites-enabled/#{user_to_config}.conf; cd /home/#{user_to_config}/ && ln -s /etc/nginx/sites-enabled/#{user_to_config}.conf #{deploy_to}/app/platform/nginx.conf"
          run "rm -f /etc/apache2/sites-enabled/#{user_to_config}.conf; cd /home/#{user_to_config}/ && ln -s /etc/apache2/sites-enabled/#{user_to_config}.conf #{deploy_to}/app/platform/apache.conf"
        end
